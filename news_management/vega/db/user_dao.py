...
class UserDao:
    ...
    def login(self,username,password):
        try:
            ...
            sql="SELECT COUNT(*) FROM t_user WHERE username=%s AND " \
                "AES_DECRYPT(UNHEX(password),'HelloWorld')=%s"                  #啊啊啊啊啊 上面记得留个空行！！
    ...
    def insert(self,username,password,email,role_id):
        try:
            ...
            cursor.execute(sql, (username,password,email,role_id))  # 假设一页十条记录
            con.commit()
    ...
    def update(self,id,username,password,email,role_id):
        try:
            ...
            sql = "UPDATE t_user SET username=%s, " \
                  "password=HEX(AES_ENCRYPT(%s,'HelloWorld')),email=%s,role_id=%s " \
                  "WHERE id=%s "
            cursor.execute(sql, (username,password,email,role_id,id))  # 假设一页十条记录
            con.commit()
        except Exception as e:
            if "con" in dir():
                con.rollback()
